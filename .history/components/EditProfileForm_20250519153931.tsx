...
export default function EditProfileForm({
  ...
  return (
    <section className="pb-6 w-full mt-6 md:mt-8 p-6 bg-gray-800 rounded-xl shadow-inner border border-gray-700">
      ...
      <form id="profile-form" onSubmit={onSubmit} className="space-y-6"> {/* Added id for sticky button */} 
        {/* Status Messages */}
        {error && <p className="text-red-400 text-sm">{error}</p>}
        {successMessage && <p className="text-green-400 text-sm">{successMessage}</p>}
         {avatarUploadError && <p className="text-red-400 text-sm">{avatarUploadError}</p>} {/* Display avatar error here too */}
