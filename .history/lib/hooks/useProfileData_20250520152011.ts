...
export default function useProfileData() {
  ...
  const handleSaveProfile = useCallback(async (e: FormEvent) => {
    ...
    try {
      console.log('[useProfileData] Attempting to save profile for user:', user.id, updatedProfileData);
      const { error: updateError } = await supabase
        .from('profiles')
      ...
      if (updateError) {
        console.error('[useProfileData] Error saving profile:', updateError);
        setError('Failed to save profile: ' + updateError.message);
        setSuccessMessage(null);
      } else {
        console.log('[useProfileData] Profile saved successfully for user:', user.id);
        ...
        setProfile(prevProfile => {
          ...
          // This case should ideally not happen if we have a user and are saving
          return updatedProfileData as Profile; 
        });
        // Optionally, instead of just updating local state, re-fetch everything:
        // await loadProfileAndSkills(user, pathname); 
        // However, be mindful of loadProfileAndSkills resetting new... states if not careful
      }
    } catch (err: any) {
      console.error('[useProfileData] Exception during profile save:', err);
    ...
    newPhoneWhatsapp, newTikTokUrl, newTelegramUrl, 
