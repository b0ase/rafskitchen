...
export default function useProfileData() {
  ...
  const loadProfileAndSkills = useCallback((currentUserParam: User, currentPathname: string) => {
    console.log('[useProfileData] loadProfileAndSkills (sync attempt) called with user:', currentUserParam?.id, 'pathname:', currentPathname);
    setLoading(true);

    console.log('[useProfileData] Fetching basic profile data (sync attempt) for user:', currentUserParam.id);
    supabase
      .from('profiles')
      .select('*')
      .eq('id', currentUserParam.id)
      .single<Profile>()
      .then(({ data: typedProfileData, error: profileError }) => {
        if (profileError) {
          console.error('[useProfileData] Error fetching profile:', profileError);
          setError('Failed to load profile: ' + profileError.message);
          setProfile(null); 
        } else if (typedProfileData) {
          console.log("[useProfileData] Profile data fetched:", typedProfileData);
          setProfile(typedProfileData);
          setNewUsername(typedProfileData.username || '');
          setNewDisplayName(typedProfileData.display_name || '');
          setNewFullName(typedProfileData.full_name || '');
          setNewBio(typedProfileData.bio || '');
          setNewWebsiteUrl(typedProfileData.website_url || '');
          setNewTwitterUrl(typedProfileData.twitter_url || '');
          setNewLinkedInUrl(typedProfileData.linkedin_url || '');
          setNewGitHubUrl(typedProfileData.github_url || '');
          setNewInstagramUrl(typedProfileData.instagram_url || '');
          setNewDiscordUrl(typedProfileData.discord_url || '');
          setNewPhoneWhatsapp(typedProfileData.phone_whatsapp || '');
          setNewTikTokUrl(typedProfileData.tiktok_url || '');
          setNewTelegramUrl(typedProfileData.telegram_url || '');
          setNewFacebookUrl(typedProfileData.facebook_url || '');
          setNewDollarHandle(typedProfileData.dollar_handle || '');
          setNewTokenName(typedProfileData.token_name || '');
          setNewSupply(typedProfileData.supply || '1,000,000,000');
        ...
        setLoading(false);
        console.log('[useProfileData] loadProfileAndSkills (sync attempt) Step 1 processed.');
      })
      .catch(e => {
        console.error("[useProfileData] Critical error in loadProfileAndSkills (sync attempt) promise chain:", e.message);
    // try { // try/catch/finally might behave differently without async/await directly here
      // Step 1: Fetch Basic Profile Data (using .then())
      console.log('[useProfileData] Fetching basic profile data (sync attempt) for user:', currentUserParam.id);
      supabase
        .from('profiles')
        .select('*')
        .eq('id', currentUserParam.id)
        .single()
        .then(({ data: rawProfileData, error: profileError }) => {
          if (profileError) {
            console.error('[useProfileData] Error fetching profile:', profileError);
            setError('Failed to load profile: ' + profileError.message);
            setProfile(null); 
          } else if (rawProfileData) {
            fetchedProfileData = rawProfileData as Profile; // This variable won't be useful outside this .then() scope easily
            console.log("[useProfileData] Profile data fetched:", rawProfileData);
            setProfile(rawProfileData as Profile);
            setNewUsername(rawProfileData.username || '');
            setNewDisplayName(rawProfileData.display_name || '');
            setNewFullName(rawProfileData.full_name || '');
            setNewBio(rawProfileData.bio || '');
            setNewWebsiteUrl(rawProfileData.website_url || '');
            setNewTwitterUrl(rawProfileData.twitter_url || '');
            setNewLinkedInUrl(rawProfileData.linkedin_url || '');
            setNewGitHubUrl(rawProfileData.github_url || '');
            setNewInstagramUrl(rawProfileData.instagram_url || '');
            setNewDiscordUrl(rawProfileData.discord_url || '');
            setNewPhoneWhatsapp(rawProfileData.phone_whatsapp || '');
            setNewTikTokUrl(rawProfileData.tiktok_url || '');
            setNewTelegramUrl(rawProfileData.telegram_url || '');
            setNewFacebookUrl(rawProfileData.facebook_url || '');
            setNewDollarHandle(rawProfileData.dollar_handle || '');
            setNewTokenName(rawProfileData.token_name || '');
            setNewSupply(rawProfileData.supply || '1,000,000,000');
            setShowWelcomeCard(true);
          } else {
            console.warn('[useProfileData] No profile data returned for user (and no error):', currentUserParam.id);
            setProfile(null);
          }
          // Since subsequent steps depend on fetchedProfileData, they would need to be chained or handled differently
          // For this test, we'll stop here within the .then() for Step 1 processing.
          setLoadingSkills(false);
          setLoadingUserTeams(false);
          setLoading(false);
          console.log('[useProfileData] loadProfileAndSkills (sync attempt) Step 1 processed.');
      ...
      // if (fetchedProfileData) { // This logic would need to be inside .then() or a subsequent .then()
        // Step 2, 3, 4 remain commented out
      // }

    // } catch (e:any) { ... } finally { ... } // Original try/catch/finally might not be ideal here without top-level await
