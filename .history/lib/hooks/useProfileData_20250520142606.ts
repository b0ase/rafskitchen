...
export default function useProfileData() {
  ...
  const loadProfileAndSkills = useCallback(async (currentUserParam: User, currentPathname: string) => {
    console.log('[useProfileData] loadProfileAndSkills called with user:', currentUserParam?.id, 'pathname:', currentPathname);
    setLoading(true);
    ...
    try {
      ...
      // if (fetchedProfileData) {
        ...
        // ... (original skill fetching logic commented out) ...

        // Step 3: Fetch User's Teams
        // console.log("[useProfileData] Fetching user teams for user:", currentUserParam.id);
        // ... (original team fetching logic commented out) ...
      // }

      // Step 4: Fetch All Available Skills for dropdown
      // console.log("[useProfileData] Fetching all available skills for dropdown...");
        // ... (original all skills fetching logic commented out) ...

    } catch (e:any) {
      console.error("[useProfileData] Critical error in loadProfileAndSkills (during Step 1 or general):", e.message);
      setError(`An unexpected error occurred: ${e.message}`);
    } finally {
      console.log('[useProfileData] loadProfileAndSkills partially finished (Step 1 attempted). States set.');
