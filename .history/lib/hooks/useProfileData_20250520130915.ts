...
export default function useProfileData() {
  ...
  useEffect(() => {
    ...
    const fetchInitialUser = async () => {
      ...
      if (isLoggingOut) {
        console.log('[useProfileData] Logout in progress, fetchInitialUser deferring auth check redirect.');
        setUser(null); 
        setLoading(false); // Stop loading indicator for this hook to prevent UI hanging
        return;
      }
      ...
      if (!fetchedUser) {
        console.log('[useProfileData] No user found with getUser() on initial load. Redirecting to login.');
        setUser(null);
        setLoading(false);
        router.push('/login');
        return;
      ...
      console.log('[useProfileData] Initial user fetched with getUser():', fetchedUser.id);
      setUser(fetchedUser);
      // setLoading(false) is typically handled after profile data is fetched, not just user object.
    };
