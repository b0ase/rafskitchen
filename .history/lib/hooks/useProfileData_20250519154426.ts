...
export default function useProfileData() {
  ...
  }, [router, supabase, user]); // Added user to dependencies as it's used within the effect.
  ...
  useEffect(() => {
    const loadProfileAndSkills = async () => {
      ...
      try {
        ...
        // setLoading(false); // Main loading state is handled by the check in the component
        setLoadingSkills(false);
        setLoadingUserTeams(false);
    ...
    if (user?.id && pathname === '/profile') {
      // setLoading(true); // Handled in the first useEffect now
