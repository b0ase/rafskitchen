...
export default function TeamPage() {
  ...
  const fetchTeamDetails = useCallback(async () => {
    ...
    // Try fetching by slug first, then by ID if slug fails (or vice-versa)
    // This assumes team slugs are unique. If not, prioritize ID.
    let teamQuery = supabase.from(\'teams\').select(\'*\');
    if (teamSlugOrId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) { // Check if it looks like a UUID
  ...
  const fetchMessages = useCallback(async (teamId: string) => {
    ...
    // setError(null); // Keep existing errors if any, or clear specifically for messages
  ...
  const handlePostMessage = async (e: FormEvent) => {
    ...
    if (insertError) {
      ...
      setNewMessageContent(\'\'); // Clear input field on success (realtime should update the list)
    }
  ...
  if (error && !teamDetails) { // Critical error if team details couldn\'t load
    return (
  ...
  if (!teamDetails) { // Should be caught by error state above, but as a fallback
      return (
          <div className="min-h-screen bg-gray-950 text-gray-300 flex flex-col items-center justify-center p-4">
