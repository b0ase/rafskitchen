...
export default function TeamPage() {
  ...
  return (
    <div className={`flex-1 flex flex-col h-full p-2 md:p-4 bg-gradient-to-br ${teamDetails.color_scheme?.bgColor || 'bg-gray-800'} to-gray-950 text-white`}>
      {/* Header with back button, team name, creator, and now member avatars */}
      <header className="p-3 border-b flex items-center justify-between space-x-3 min-h-[70px] sticky top-0 z-10 bg-opacity-80 backdrop-blur-md" style={{ borderColor: teamDetails?.color_scheme?.borderColor || 'border-gray-700', backgroundColor: teamDetails?.color_scheme?.bgColor ? `${teamDetails.color_scheme.bgColor}CC` : 'rgba(31, 41, 55, 0.8)' }}>
                ...
                <div className="flex items-center pl-2">
                  {loadingMembers ? (
                    ...
                    teamMembers.map((member, index) => (
                      <div key={member.id} title={member.displayName} className={`w-7 h-7 md:w-8 md:h-8 rounded-full border-2 ${teamDetails?.color_scheme?.borderColor || 'border-gray-600'} overflow-hidden ${index > 0 ? '-ml-2 md:-ml-3' : ''} bg-gray-700 flex items-center justify-center text-xs font-semibold`}>
                        {member.avatarUrl ? (
