    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // The payload.new should already have the basic message fields.
          // We need to fetch the profile separately if it's not included or to ensure correct structure.
          const newMessageRaw = payload.new as Omit<Message, 'profiles'>; // Base message type
          ...
          if (newMessageRaw.user_id) {
            ...
            if (profileError) {
              ...
              setMessages(currentMessages => [...currentMessages, { ...newMessageRaw, profiles: profileData as ProfileForMessage | null }]);
            }
          } else {
            // Fallback if user_id is somehow missing in the new message payload
            setMessages(currentMessages => [...currentMessages, { ...newMessageRaw, profiles: null }]);
          }
        }
      )
