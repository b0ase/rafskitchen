...
export default function TeamPage() {
  ...
  const fetchTeamDetails = useCallback(async () => {
    ...
    // Await directly on the .single() call should resolve the PostgrestBuilder issue.
    // The .single() method modifies the builder to expect a single object or null.
    ...
    if (teamError || !data) {
      ...
      // Cast data to 'any' to handle dynamic properties before assigning to typed state
      const teamData = data as any;
      setTeamDetails({
        id: teamData.id, // Ensure required fields are explicitly mapped
        name: teamData.name, // Ensure required fields are explicitly mapped
  ...
  const fetchMessages = useCallback(async (teamId: string) => {
    ...
    if (messagesError) {
      ...
      // Assuming profiles from the join is a single object, not an array.
      // If it can be an array, Message interface or this mapping needs adjustment.
      setMessages(data.map(m => ({ ...m, profiles: m.profiles as { display_name: string | null; avatar_url?: string | null; } | null })) as Message[]);
    }
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (fetchError) {
            ...
            // Cast the new message similarly
            const properlyTypedMessage = { 
              ...(newMessageWithProfile as any), 
              profiles: (newMessageWithProfile as any).profiles as { display_name: string | null; avatar_url?: string | null; } | null 
            } as Message;
            setMessages(currentMessages => [...currentMessages, properlyTypedMessage]);
          }
        }
      )
      .subscribe();
  ...
  const handleLeaveTeam = async () => {
    ...
    try {
      ...
      alert('Successfully left the team!'); // Or use a more integrated notification system
      router.push('/profile'); // Redirect to profile page or another relevant page
    } catch (err: any) {
          ...
          {currentUser && teamDetails && (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors 
                          ${textColor} bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed border ${textColor}`}
            >
              {leavingTeam ? (
