...
export default function TeamPage() {
  ...
  return (
    ...
    >
            ...
            <footer className={`w-full p-4 sticky bottom-0 ${teamDetails.color_scheme?.bgColor || 'bg-gray-800'} border-t ${teamDetails.color_scheme?.borderColor || 'border-gray-700'}`}>
              <form onSubmit={handlePostMessage} className="container mx-auto flex items-center space-x-3">
                ...
                <button 
                  ...
                  className={`px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold flex items-center justify-center shadow-md transition-colors 
      ...
      {/* Members Section */}
      <div className="w-full md:w-1/3 lg:w-1/4 p-4 bg-gray-800 dark:bg-gray-900 border-l border-gray-700 dark:border-gray-800">
        ...
        {loadingMembers && <p className="text-gray-400 dark:text-gray-500">Loading members...</p>}
        
        {/* Mobile Dropdown Content */}
        <div id="members-list" className={`md:hidden ${showMembersDropdown ? 'block' : 'hidden'}`}>
          {teamMembers.length > 0 ? (
            <ul className="space-y-2">
              {teamMembers.map((member) => (
                <li key={member.id} className="flex items-center space-x-2 p-2 bg-gray-700 dark:bg-gray-800 rounded-md">
                  {member.avatarUrl ? (
                  ...
                  <span className="text-sm text-gray-200 dark:text-gray-300 truncate">{member.displayName}</span>
                </li>
              ))}
            </ul>
          ) : (
            !loadingMembers && <p className="text-gray-400 dark:text-gray-500 text-sm">No members found.</p>
          )}
        </div>
