    ...
    const fetchTeams = async () => {
      ...
      if (fetchError) {
        ...
        const processedData = data.map((team: any) => {
          ...
          // Supabase might return an array for the joined 'profiles'. Take the first if it exists.
          if (Array.isArray(team.profiles) && team.profiles.length > 0) {
            creatorProfile = team.profiles[0];
          } else if (team.profiles && !Array.isArray(team.profiles)) {
            // If it's an object and not an array (older Supabase client behavior or direct FK object result)
            creatorProfile = team.profiles;
          }
          ...
          return {
            ...
            profiles: creatorProfile // Assign the correctly shaped profile object or null
          };
        });
