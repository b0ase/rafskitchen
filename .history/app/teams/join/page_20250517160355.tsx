  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      const { data, error: fetchError } = await supabase
        .from('teams')
        .select('id, name, slug, description, icon_name, color_scheme, profiles:created_by (display_name, avatar_url)')
      ...
      if (fetchError) {
        ...
        const processedData: Team[] = data.map((team: any) => {
          ...
          // team.profiles will be an object if created_by has a matching profile, or null otherwise.
          const creatorProfile: Team['profiles'] = 
            (team.profiles && typeof team.profiles === 'object' && !Array.isArray(team.profiles))
            ? {
                display_name: team.profiles.display_name || null,
