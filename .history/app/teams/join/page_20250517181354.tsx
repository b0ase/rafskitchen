...
export default function JoinTeamPage() {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      if (fetchError) {
        console.error('Error fetching teams on /teams/join page (simplified query):', JSON.stringify(fetchError, null, 2));
        ...
        const processedData: Team[] = data.map((team: any) => {
          ...
          const userObject = team.created_by;
          let creatorProfile: Team['creator_profile'] = null; // Explicitly null for this test
          let creatorDisplayName = null;
          let creatorAvatarUrl = null;

          // Attempt to get display name and avatar from raw_user_meta_data if available
          if (userObject?.raw_user_meta_data) {
            const meta = userObject.raw_user_meta_data;
            creatorDisplayName = meta.name || meta.user_name || meta.full_name || 'User';
            creatorAvatarUrl = meta.avatar_url || null;
            // Construct a partial creator_profile if we got something
            if (creatorDisplayName || creatorAvatarUrl) {
                creatorProfile = {
            ...
            creatorProfile = { display_name: userObject.email, avatar_url: null }; // Fallback to email
          }
          ...
          return {
            ...
            creator_profile: creatorProfile, // Will be null or from raw_user_meta_data
