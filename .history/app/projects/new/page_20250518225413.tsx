...
export default function NewProjectPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (insertError) {
      ...
      // --- BEGIN ADDING CREATOR TO project_users ---
      if (user && user.id) { // user should be defined here due to earlier check
        const { error: projectUserError } = await supabase
          .from('project_users')
          .insert({
            project_id: data.id, // data is the newProjectData from the select()
            user_id: user.id,
            role: ProjectRole.ProjectManager
          });

        if (projectUserError) {
          console.error('Error adding project creator to project_users:', projectUserError);
          // This is a secondary error. The project was created.
          // You might want to set a different kind of message or log this for follow-up.
          setSuccessMessage('Project created, but there was an issue assigning ownership role. Please check team settings or contact support.');
        } else {
          setSuccessMessage('Project created successfully! Redirecting...');
        }
      } else {
        // Should not happen if user check at start of handleSubmit is effective
        console.warn('User became undefined before assigning project role.');
        setSuccessMessage('Project created, but user session issue occurred before role assignment.');
      }
      // --- END ADDING CREATOR TO project_users ---
      
      localStorage.removeItem('pendingProjectData'); // Clear data on success
      router.push(`/myprojects/${data.project_slug}`); 
    }
  };
