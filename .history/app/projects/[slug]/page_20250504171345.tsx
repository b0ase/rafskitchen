        ...
        <>
          ...
          <section className="mb-8" key="features-section">
            ...
            <div className="mt-6 pt-6 border-t border-gray-700">
              <h3 className="text-lg font-semibold mb-3 text-white">Request a New Feature</h3>
              <form onSubmit={handleNewFeatureSubmit} className="flex flex-col sm:flex-row gap-3">
                <input
                  type="text"
                  placeholder="Feature Name / Description"
                  value={newFeatureForm.name}
                  onChange={(e) => setNewFeatureForm(f => ({ ...f, name: e.target.value }))}
                  className="px-3 py-2 bg-gray-800 border border-gray-700 rounded w-full sm:flex-grow text-white" 
                  required
                />
                <select
                  value={newFeatureForm.priority}
                  onChange={(e) => setNewFeatureForm(f => ({ ...f, priority: e.target.value }))}
                  className="px-3 py-2 bg-gray-800 border border-gray-700 rounded w-full sm:w-auto text-white"
                >
                ...
                <button
                  ...
                  className="bg-green-700 hover:bg-green-600 text-white font-bold py-2 px-4 rounded disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? 'Submitting...' : 'Submit Request'}
                </button>
              </form>
              {newFeatureSuccess && <p className="text-green-400 mt-2">{newFeatureSuccess}</p>}
            </div>
