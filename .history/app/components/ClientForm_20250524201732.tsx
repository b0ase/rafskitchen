  ...
  return (
    <div className="max-w-lg mx-auto my-8 p-6 bg-gray-900 text-white rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        {form.logo_url && (
          <Image src={form.logo_url} alt="Logo preview" className="mt-2 h-12 rounded bg-white object-contain" width={48} height={48} />
        )}
        <button
          type="submit"
        ...
        <ul className="space-y-2">
          {clients.map(client => (
            <li key={client.id} className="bg-gray-800 p-3 rounded flex items-center gap-3">
              {client.logo_url && (
                <Image src={client.logo_url} alt="logo" className="w-8 h-8 rounded-full object-cover" width={32} height={32} />
              )}
              <div>
