      ...
      <nav className="flex-grow">
        <ul className="space-y-1.5">
          {navLinks.map((link) => {
            ...
            // A more specific check for /myprojects to ensure it's highlighted for /myprojects/*
            // For /myprojects, we want it active if pathname is /myprojects OR /myprojects/anything
            ...
            if (link.href === '/myprojects') {
              ...
              // For other general links, exact match or if it's a parent path
              // (e.g. href /gigs should be active for /gigs/calendar)
              // but ensure href itself is not just '/' to avoid highlighting home for all paths
              if (link.href !== '/') {
                isLinkActive = pathname === link.href || pathname.startsWith(link.href + '/'); 
              } else {
                 isLinkActive = pathname === '/'; // Home is only active for exact match
              }
            }
