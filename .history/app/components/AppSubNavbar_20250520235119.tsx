...
export default function AppSubNavbar({ initialIsExpanded, onCollapse }: AppSubNavbarProps) {
  ...
  const linkContainerClasses = isExpanded 
    ...
    : (isDesktop 
      ...
      : 'flex items-center space-x-2 overflow-x-auto whitespace-nowrap scrollbar-hide py-1'); // Mobile collapsed: scroll all
          ...
          <div className="flex justify-center py-2">
            ...
            >
              {isExpanded ? (
                ...
                <>
                  <FaChevronDown className="h-4 w-4 text-gray-400 mr-2" /> 
                  <span className="text-sm font-medium">Show More Options</span>
                </>
              )}
            </button>
          </div>
        ...
        <div className={`flex items-center justify-between py-2.5 sm:py-3`}> 
          ...
          <div className={`flex-grow ${linkContainerClasses}`}>
            {displayedLinks.map((link, index) => {
              ...
              const buttonBaseClasses = `px-3 py-3 rounded-md text-sm font-medium transition-colors duration-150 ease-in-out`;
