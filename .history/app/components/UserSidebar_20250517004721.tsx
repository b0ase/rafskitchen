  ...
  const handleSidebarAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (publicUrlData?.publicUrl) {
        ...
        // Ensure the URL doesn't accidentally contain '/public/public/' if filePath itself starts with 'public/'
        // and getPublicUrl also prefixes with the bucket content structure.
        // A typical correct URL: <base>/avatars/public/user_id/avatar.png
        // A potential issue if filePath = "public/..." and getPublicUrl makes it <base>/avatars/public/public/...
        // However, Supabase's getPublicUrl is generally smart enough to handle this if filePath is just the object path.

        // The filePath IS public/user_id/avatar.ext.
        // So, publicUrlData.publicUrl should be <base_storage_url>/avatars/public/user_id/avatar.ext
        // This seems correct and usually doesn't need further manipulation unless Supabase has a known bug
        // or the interpretation of filePath by getPublicUrl is tricky.

        // For now, let's assume getPublicUrl with a filePath starting with "public/" gives the correct full public path.
        // The previous correction logic was problematic.

        const newPublicAvatarUrlWithCacheBust = `${finalPublicUrl}?t=${newTimestamp}`;
        ...
        // This case should ideally not happen if upload succeeds and bucket is public.
        console.error('[UserSidebar] Avatar Upload: Could not get public URL even after successful upload.');
