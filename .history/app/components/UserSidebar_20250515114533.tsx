...
export default function UserSidebar({ onSetPageTitle }: UserSidebarProps) {
  ...
  const allNavLinks = React.useMemo(() => [...navLinksPrimary, ...navLinksSecondary], []); // navLinksPrimary and navLinksSecondary should be stable
  ...
  useEffect(() => {
    let currentTitle = 'App'; // Default title

    // Rule 1: Exact match or /profile at root path
    ...
    if (exactMatch) {
      currentTitle = exactMatch.title;
    } else {
      // Rule 2: Most specific partial match for nested routes
      // Filter links that are a prefix of the current pathname (e.g., link.href = /gigs, pathname = /gigs/calendar)
      // Ensure href is not just '/' to avoid matching everything.
      ...
      if (partialMatches.length > 0) {
        ...
        currentTitle = partialMatches[0].title;
      } 
      // Rule 3: Specific fallback for root if no other match (e.g. if /profile isn't matched by rule 1 but should be the default for '/')
      else if (pathname === '/') {
        const profileLink = allNavLinks.find(link => link.href === '/profile');
        if (profileLink) {
          currentTitle = profileLink.title;
        } else {
          currentTitle = 'Dashboard'; // Or another sensible default for root
        }
      }
      // If currentTitle is still 'App', it means no specific rule matched, so 'App' remains the generic fallback.
    }
    onSetPageTitle(currentTitle);
  }, [pathname, onSetPageTitle, allNavLinks]);
