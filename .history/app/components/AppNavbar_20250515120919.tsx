...
export default function AppNavbar({ pageContext }: AppNavbarProps) {
  ...
  if (pageContext) {
    ...
    breadcrumbSegments.push({ text: formatBreadcrumbSegment(pathname.replace(/^\/+/, '') || "App"), href: pathname });
  }
    ...
    <nav className="bg-gray-900 text-white p-4 shadow-md h-16 flex items-center">
      <div className="container mx-auto flex items-center">
        <div className="text-xl font-semibold flex items-center">
          {IconToShow && <IconToShow className="mr-3 h-5 w-5 flex-shrink-0 text-sky-500" />}
          {breadcrumbSegments.map((segment, index) => (
            <React.Fragment key={segment.href}>
              {index > 0 && <span className="text-gray-500 font-normal text-lg mx-1">/</span>}
              {index === breadcrumbSegments.length - 1 && breadcrumbSegments.length > 1 ? (
