...
export default function UserSidebar() {
  ...
  const handleLogout = async () => {
    console.log('[UserSidebar] Attempting logout...');
    const { error } = await supabase.auth.signOut();
    
    if (error) {
      console.error('[UserSidebar] Error signing out:', error.message);
      // Optionally, you could set some state here to show an error message to the user
    } else {
      console.log('[UserSidebar] Sign out successful, redirecting to /');
      // Using window.location.assign to force a full page reload from the server
      // This can be more reliable for clearing all auth-related state.
