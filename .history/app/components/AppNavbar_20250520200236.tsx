...
interface AppNavbarProps {
  // toggleFullScreenMenu: () => void; // To be removed
  // isFullScreenMenuOpen: boolean; // To be removed
}
...
export default function AppNavbar({ /* toggleFullScreenMenu, isFullScreenMenuOpen */ }: AppNavbarProps) {
  ...
  const [isMobileDropdownOpen, setIsMobileDropdownOpen] = React.useState(false); // New state

  const toggleMobileDropdown = () => { // New toggle function
  ...
  return (
    <nav className="bg-black text-white p-4 shadow-md h-28 flex items-center sticky top-0 z-40">
      <div className="container mx-auto flex items-center justify-between">
        <button 
          onClick={toggleMobileDropdown} // Changed to local toggle
          className="text-gray-400 hover:text-white focus:outline-none md:hidden mr-3"
          aria-label={isMobileDropdownOpen ? "Close menu" : "Open menu"} // Changed state variable
        >
          {isMobileDropdownOpen ? <FaTimes className="w-7 h-7" /> : <FaBars className="w-7 h-7" />} {/* Changed state variable */}
        </button>
        ...
        <div>
          <Link href="/settings" legacyBehavior>
            <a className="text-gray-400 hover:text-white transition-colors" title="Settings">
