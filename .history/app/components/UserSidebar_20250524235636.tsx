...
export default function UserSidebar({ className }: UserSidebarProps) { // Destructure className
  ...
  return (
    <div className={`flex flex-col h-full w-64 bg-black text-white ${className}`}>
      ...
      <div className="px-6 py-4 border-b border-gray-700">
        <div className="flex items-center space-x-3">
          {userAvatarUrl ? (
            <Image 
              src={userAvatarUrl} 
              alt={`${userDisplayName || 'User'}'s avatar`} 
              className="w-10 h-10 rounded-full object-cover border-2 border-gray-600" 
              width={40} 
              height={40} 
            />
          ) : userInitial ? (
            <div className="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center text-white text-lg font-semibold border-2 border-gray-600">
              {userInitial}
            </div>
          ) : (
            <div className="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center text-white text-lg font-semibold border-2 border-gray-600">
              {userInitial || 'U'}
            </div>
          )}
          <div className="truncate">
            <p className="font-semibold text-sm leading-tight truncate" title={userDisplayName || 'User'}>{userDisplayName || 'Loading...'}</p>
            {user && user.email && (
              <p className="text-xs text-gray-400 leading-tight truncate" title={user.email}>{user.email}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mt-auto px-2 py-4 border-t border-gray-700 space-y-1">
