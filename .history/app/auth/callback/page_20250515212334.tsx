...
export default async function CallbackPage({ searchParams }) {
  ...
  if (code) {
    ...
    if (!error) {
      ...
      return NextResponse.redirect(new URL(redirectTo, process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000')).toString(); // Return the URL as a string
    }
  }

  // If there's no code or an error, redirect to the login page with an error parameter
  return NextResponse.redirect(new URL('/login?error=authentication_failed', process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000')).toString(); // Return the URL as a string
} 