...
export default function FinancesPage() {
  ...
  const fetchAllFinancialData = useCallback(async (userId: string) => {
    ...
    try {
      ...
      // Define the select string for transactions carefully
      const transactionSelect = `
        id, user_id, financial_account_id, transaction_category_id, transaction_date, description, amount, currency_code, merchant_name, status, notes, is_recurring, source_data, created_at, updated_at,
        financial_account:financial_accounts!inner(name),
        transaction_category:transaction_categories!inner(name, icon_name, color_hex)
      `;
      ...
      // Destructure with type safety in mind (example, actual types might vary based on PostgrestResponse)
      const accountsResponse = promiseResults[0] as { data: FinancialAccount[] | null; error: any | null };
      const transactionsResponse = promiseResults[1] as { data: (Omit<Transaction, 'account_name' | 'category_name' | 'category_icon' | 'category_color'> & { financial_account: { name: string } | null; transaction_category: { name: string; icon_name: string | null; color_hex: string | null; } | null; })[] | null; error: any | null };
      ...
      const processedTransactions = (transactionsResponse.data || []).map(tx => {
        return {
          ...tx,
          account_name: tx.financial_account?.name || 'N/A',
          category_name: tx.transaction_category?.name || 'Uncategorized',
          category_icon: tx.transaction_category?.icon_name,
          category_color: tx.transaction_category?.color_hex,
          financial_account: undefined, // Remove the nested object after processing
          transaction_category: undefined, // Remove the nested object after processing
