  ...
  const fetchAllFinancialData = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const processedTransactions = (transactionsResponse.data || []).map(tx => {
        const financialAccount = tx.financial_account as { name: string } | null; // Type assertion for joined data
        const transactionCategory = tx.transaction_category as { name: string; icon_name: string | null; color_hex: string | null } | null;
        return {
          ...tx,
          ...
          account_name: financialAccount?.name || 'N/A',
          category_name: transactionCategory?.name || 'Uncategorized',
          category_icon: transactionCategory?.icon_name,
          category_color: transactionCategory?.color_hex,
        };
      });
