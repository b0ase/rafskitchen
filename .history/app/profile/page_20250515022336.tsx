...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchInitialUser = async () => {
      ...
      setUser(fetchedUser);
      // setLoading(false) will be handled after profile data is attempted or if user is null
    };
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      // Update user state immediately based on the event
      // This ensures 'user' state is fresh before dependent effects run
      ...
      if (event === 'SIGNED_OUT') {
        ...
        // setLoading(true); // To show loading during redirect transition
        router.push('/login');
      } else if (event === 'SIGNED_IN' && currentUser) {
        console.log('[ProfilePage] User signed in:', currentUser.id);
        // Profile fetching will be handled by the useEffect dependent on `user` state.
        // setLoading(true) // If we want loading spinner during profile fetch for new sign in
      } else if (event === 'USER_UPDATED' && currentUser) {
        console.log('[ProfilePage] User updated:', currentUser.id);
        // Profile fetching will be handled by the useEffect dependent on `user` state.
      }
      // If user becomes null for other reasons (e.g. token expired and not refreshed)
      // and it's not 'SIGNED_OUT', the `user` state will be updated to null,
      // and the dependent useEffect will clear profile.
  ...
  }, [router]); // Added user to deps to re-run if user object identity changes externally, though primarily driven by onAuthStateChange and getUser

  // NEW useEffect for fetching profile and skills when user is available or changes
  useEffect(() => {
    const loadProfileAndSkills = async () => {
      if (!user?.id) {
        // If there's no user, ensure profile is null and stop loading.
        // This can happen if initial fetch fails or user signs out.
        setProfile(null);
        if (router.pathname === '/profile') { // Only set loading false if we are on profile page and expect data or an error
             setLoading(false);
        }
        return;
      }
      ...
      setLoading(true); // Start loading when we attempt to fetch profile data
      setError(null);   // Clear previous errors

      try {
        // Fetch Profile with User Skills
        ...
        if (profileError) {
          ...
          if (profileError.code === 'PGRST116') {
            ...
            // Optionally, pre-fill some fields or guide user
          } else {
        ...
        // Fetch All Skills (for the dropdown) - This can be independent or moved if already fetched
        ...
        if (allSkillsError) {
          ...
          //setError(prevError => prevError ? prevError + " | Failed to load skill list." : "Failed to load skill list.");
        } else {
    ...
    if (user) {
      ...
      // If user becomes null (e.g., after sign out and router.push hasn't completed or if already on /login)
      // ensure profile is cleared and loading stops.
      setProfile(null);
      setLoading(false); 
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, supabase, router]); // supabase and router are stable dependencies, user is the primary trigger
