...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadProfileAndSkills = async () => {
      if (!user?.id) { // Should be captured by the outer conditional, but good safeguard
        setProfile(null);
        setSelectedSkills([]);
        setLoading(false); 
      ...
      console.log(`[ProfilePage] User ID ${user.id} present. Starting to fetch profile and skills.`);
      ...
      try {
        ...
        // Step 3: Fetch All Available Skills for dropdown
        ...
        setLoading(false); 
        setLoadingSkills(false); 
        console.log(`[ProfilePage] loadProfileAndSkills finished. States set: main loading = false, skills loading = false.`);
      }
    };
  ...
  if (loading || !user) {
    console.log(`[ProfilePage] Render: Main Loading screen (loading: ${loading}, user: ${!!user})`);
    return (
