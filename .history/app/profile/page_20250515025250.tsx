    ...
    const loadProfileAndSkills = async () => {
      ...
      try {
        ...
        if (profileError) {
          ...
          // setProfile(null); // Already cleared
        } else if (profileData) {
          ...
          if (userSkillsError) {
            ...
            const currentSkills = userSkillsData.map(us => {
              // Ensure us.skills is treated as an object, which it should be from the select query
              const skillDetail = us.skills as Skill; // Type assertion, assuming Skill interface matches structure
              if (!skillDetail) {
                console.warn("[ProfilePage] User skill entry found without corresponding skill detail:", us);
                return null; // Or handle as appropriate, e.g., skip or default
              }
              return {
                id: skillDetail.id, 
            ...
            }).filter(skill => skill !== null) as Skill[]; // Filter out any nulls and assert final type
        ...
        // Step 3: Fetch All Skills (for the dropdown) - This can remain as is
        ...
        if (allSkillsError) {
          ...
          // Optionally set an error for this if critical, or just log it
        } else {
        ...
        setLoading(false);
        console.log("[ProfilePage] loadProfileAndSkills finished. Loading state:", loading);
      }
    };
