        ...
        <section className="mb-10 pb-6 border-b border-gray-700">
          ...
          {loadingSkills ? (
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              {/* Always-visible Inline Skill Adder Dropdown (if skills available and not saving) */}
              {!savingSkills && allSkills.filter(skill => !userSkillIds.has(skill.id)).length > 0 && (
                  ...
                  <select 
                    ...
                    onChange={async (e) => {
                      ...
                      if (selectedValue && user) {
                        setSkillChoiceInAdder(selectedValue); // Update state to reflect selection if needed for any reason
                        await handleSkillToggle(selectedValue, false); // Add the skill
                        setSkillChoiceInAdder(''); // Reset select to placeholder for next addition
                      } else if (!user) {
                        setError('User not authenticated.');
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold rounded-full shadow-md appearance-none min-w-[150px] focus:outline-none focus:ring-2 focus:ring-sky-500 transition-colors
                                ${skillChoiceInAdder === '' ? 'bg-gray-700 text-gray-400 hover:bg-gray-600' : 'bg-gray-800 text-gray-300'} `}
                  >
                    <option value="" disabled={skillChoiceInAdder !== ''} className="text-gray-500">+ Add Skill</option>
                    {Object.entries(
                      allSkills
                        .filter(skill => !userSkillIds.has(skill.id))
                        .reduce((acc, skill) => {
              ...
              {userSkillIds.size > 0 && allSkills.filter(skill => !userSkillIds.has(skill.id)).length === 0 && (
