...
export default function UserProfilePage() {
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    if (skillsError) {
      ...
      // Non-critical, so don't block profile display if skills fail to load
      setUserSkills([]);
    } else {
      // Adjust processing based on the assumption that us.skills is an object, not an array.
      // Filter out any entries where us.skills might be null or undefined first.
      const validSkills = skillsData
        ?.map(us => us.skills) // This should give an array of Skill objects (or nulls)
        .filter(skill => skill !== null && typeof skill === 'object' && skill.id && skill.name) as Skill[] || [];
      setUserSkills(validSkills);
    }
