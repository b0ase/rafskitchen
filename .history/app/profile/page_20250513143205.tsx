...
export default function ProfilePage() {
  ...
  const handleUpdateProfile = async (e: FormEvent) => {
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ username: sanitizedUsername, updated_at: new Date().toISOString() })
      .eq('id', user.id);
    ...
    if (updateError) {
      ...
      setNewUsername(sanitizedUsername); // Update the input field to show the sanitized version
