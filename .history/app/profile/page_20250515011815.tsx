...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (fileToUpload: File) => {
    ...
    try {
      ...
      setUploadingAvatar(false); // Stop loading indicator on success

    } catch (e: any) {
      console.error('Error uploading avatar:', e);
      setError(`Failed to upload avatar: ${e.message}. Check console for details.`);
      setUploadingAvatar(false); // Stop loading indicator on error
    } finally {
      console.log('[handleAvatarUpload] Upload process finished. Uploading state:', uploadingAvatar);
      // The setUploadingAvatar(false) calls in the try and catch blocks are responsible for stopping the spinner.
