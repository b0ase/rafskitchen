...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadProfileAndSkills = async () => {
      if (!user?.id) {
        setProfile(null);
        setSelectedSkills([]); // Ensure skills are cleared if no user
        if (pathname === '/profile') { 
             setLoading(false);
        }
        return;
      ...
      setLoading(true); 
      setError(null);   
      setProfile(null); // Clear previous profile before fetching
      ...
      try {
        ...
        const { data: profileData, error: profileError } = await supabase
          .from('profiles')
          .select('*') // Fetch all columns from profiles table directly
          .eq('id', user.id)
          .single();
        ...
        if (profileError) {
          ...
          setLoadingSkills(false); // If profile fails, we also stop loading skills section
        } else if (profileData) {
          ...
          // Step 2: Fetch User's Skills (now that profile is confirmed or attempted)
          ...
          if (userSkillsError) {
            ...
            // Update userSkillIds Set as well
            setUserSkillIds(new Set(currentSkills.map(skill => skill.id))); 
            console.log("[ProfilePage] User skills set (selectedSkills and userSkillIds populated):", currentSkills);
          } else {
            console.log("[ProfilePage] No skills data found for user.");
            // setSelectedSkills([]); // Already cleared
          }
          // setLoadingSkills(false) will be handled after allSkills are also fetched or attempted
        } // End of if (profileData)

        // Step 3: Fetch All Skills (for the dropdown)
        ...
        if (allSkillsError) {
          ...
          console.log("[ProfilePage] All skills for dropdown fetched.");
        }
        // Now that all skill-related fetching is done (user's selected skills and all skills for dropdown)
        setLoadingSkills(false);

      } catch (e: any) {
        console.error("[ProfilePage] Exception in loadProfileAndSkills:", e);
        setError(`An unexpected error occurred: ${e.message}`);
        setLoadingSkills(false); // Also ensure it's false in case of a general exception
      } finally {
        setLoading(false); // This is for the main page loading state
        // setLoadingSkills(false); // Moved it earlier to be more specific to skills loading cycle
        console.log("[ProfilePage] loadProfileAndSkills finished. Main loading state:", loading, "Skills loading state:", loadingSkills);
      }
    };
