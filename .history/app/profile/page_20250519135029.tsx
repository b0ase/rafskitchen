...
interface UserSkill {
  ...
  user_id: string;
  skill_id: string;
  skills: Skill; // Changed from Pick<Skill, 'id' | 'name' | 'category'> to full Skill type
}
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const loadSkills = async () => {
      if (!user?.id) {
        setLoadingSkills(false); // Reset loading if no user
        return;
      }
      ...
      try {
        ...
        if (userSkillsError) {
          ...
          setLoadingSkills(false); // Reset loading on error
          return;
        }
  ...
  useEffect(() => {
    ...
    const loadTeams = async () => {
      if (!user?.id) {
        setLoadingUserTeams(false); // Reset loading if no user
        return;
      }
      ...
      try {
        ...
        if (teamUserError) {
          setLoadingUserTeams(false); // Reset loading on error
          throw teamUserError;
        }
  ...
  const handleUpdateProfile = async (e: FormEvent) => {
    ...
    if (updateError) {
      ...
      setProfile(prevProfile => {
        ...
        const updatedProfileState: DatabaseProfile = {
      ...
      <main className="flex-grow bg-black">
        <div className="mx-auto bg-black">
          ...
          <div className="mt-10">
            ...
            <section className="mt-8 pb-6 w-full bg-gray-800/30 p-4 md:p-6 rounded-xl border border-gray-700 shadow-lg">
              <h2 className="text-xl font-semibold text-sky-400 hover:text-sky-300 transition-colors duration-150 flex items-center mb-5">
