...
export default function ProfilePage() {
  ...
  // --- NEW Handler for saving from inline adder ---
  const saveNewSkillFromAdder = async () => {
    if (skillChoiceInAdder && user) {
      await handleSkillToggle(skillChoiceInAdder, false); 
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              {showSkillAdderDropdown ? (
                <div className="flex items-center gap-2 p-1 bg-gray-800 border border-gray-700 rounded-md shadow-md animate-fadeInQuickly">
                  ...
                  <button
                    onClick={saveNewSkillFromAdder}
                    disabled={!skillChoiceInAdder || savingSkills}
                    className="p-1.5 bg-green-600 hover:bg-green-500 text-white rounded-md text-xs disabled:opacity-50"
                ...
                allSkills.filter(skill => !userSkillIds.has(skill.id)).length > 0 && !savingSkills && (
                  <button 
                      ...
                      className="px-3 py-1.5 text-xs font-semibold bg-sky-600 hover:bg-sky-500 text-white rounded-full shadow-md transition-colors transform hover:scale-105 disabled:opacity-50"
