        ...
        <section id="projects" className="mb-16 md:mb-24 scroll-mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.filter(p => p.type === 'domain').map((project) => (
              ...
              >
                <div className="relative w-full h-40 bg-gray-200 dark:bg-gray-700 transition-opacity duration-300 group-hover:opacity-30">
                  <ProjectCardImage 
                    imageUrls={project.cardImageUrls || []} 
                    alt={`${project.title} screenshot`}
                  />
                  {project.slug === 'ninjapunkgirls-com' && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
                      <span className="text-white text-xl font-semibold tracking-wider">Coming Soon</span>
                    </div>
                  )}
                </div>
