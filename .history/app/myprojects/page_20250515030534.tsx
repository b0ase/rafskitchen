  ...
  return (
    <div 
      ...
      onClick={(e) => {
        ...
        router.push(`/myprojects/${project.project_slug}`);
      }}
      role="link"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <Link href={`/myprojects/${project.project_slug}`} passHref legacyBehavior tabIndex={-1}>
        <a className="block w-full h-full p-6 rounded-lg focus:outline-none" 
           aria-hidden="true"
        >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mt-auto pt-3 border-t border-slate-700/50">
            <div className="flex items-center gap-x-2">
              <input 
                ...
                disabled={updatingItemId === project.id}
                className="form-checkbox h-4 w-4 text-sky-600 bg-gray-800 border-gray-600 rounded focus:ring-sky-500 transition duration-150 ease-in-out relative z-10"
                onClick={(e) => e.stopPropagation()} 
              />
