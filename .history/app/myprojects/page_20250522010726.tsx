  ...
  const fetchProjectsAndRoles = useCallback(async () => {
    ...
    try {
      ...
      const { data: ownedProjectsData, error: ownedError } = await supabase
        .from('projects')
        .select('id, name, slug, status, badge1, badge2, badge3, badge4, badge5, is_featured, live_url, owner_user_id, created_at, is_public, client_id, clients ( user_id, name, email )')
      ...
      const { data: clientProjectsData, error: clientError } = await supabase
        .from('projects')
        .select('id, name, slug, status, badge1, badge2, badge3, badge4, badge5, is_featured, live_url, owner_user_id, created_at, is_public, client_id, clients!inner ( user_id, name, email )')
