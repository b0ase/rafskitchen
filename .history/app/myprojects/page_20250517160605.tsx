...
interface ClientProject {
  ...
  user_id: string; // This is the original creator_id from clients table
  website?: string | null; 
  // user_role_in_project?: string; // Optional: if we want to use the role directly on the project object
}
...
export default function MyProjectsPage() {
  ...
  const fetchUserAndProjects = async () => {
    ...
    if (authUser) {
      setUser(authUser);
      // Fetch projects through the user_project_memberships table
      const { data: projectMembershipData, error: projectError } = await supabase
        .from('user_project_memberships')
        .select(`
          role,
          clients (
            id, name, project_slug, status, project_brief, badge1, badge2, badge3, is_featured, badge4, badge5, user_id, website
          )
        `)
      ...
      if (projectError) {
        ...
        setProjects([]);
      } else if (projectMembershipData) {
        // Filter out null clients (if a membership exists for a deleted client)
        // And map to the ClientProject structure
        const userProjects = projectMembershipData
          .filter(membership => membership.clients !== null)
          .map(membership => {
            // Explicitly cast to ensure 'clients' is not an array. 
            // A direct join like this should return clients as an object.
            const clientData = membership.clients as unknown as ClientProject; 
            return {
              ...clientData, // Spread all fields from clients table record
              // If you need the role, you can add it to ClientProject interface and here:
              // user_role_in_project: membership.role 
            };
          });
        setProjects(userProjects as ClientProject[]); // Cast to ClientProject[]
      } else {
        setProjects([]); // No memberships found
      }
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-black to-gray-950 text-gray-300 flex flex-col">
      <main className="flex-grow container mx-auto px-4 py-12 md:py-16">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10">
            <div className="flex flex-wrap gap-4"> {/* Wrapper for buttons */}
                {user && (
                ...
                {user && (
                    <Link href="/projects/join" passHref legacyBehavior>
                        <a className="inline-flex items-center bg-sky-600 hover:bg-sky-500 text-white font-semibold py-2 px-4 rounded-md transition-colors shadow-md hover:shadow-lg">
            ...
            {/* If there were other elements on the right side, they would go here or outside the div if it should span full width */}
        </div>
