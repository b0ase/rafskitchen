...
export default function MyProjectsPage() {
  ...
  const { user, isLoading: authLoading } = useAuth(); // Using useAuth hook
  ...
  const fetchProjectsAndRoles = useCallback(async () => {
    ...
    try {
      ...
      const { data: ownedProjectsData, error: ownedError } = await supabase
        .from('projects')
        .select(\'''
          id, name, slug, status, project_brief, 
          badge1, badge2, badge3, badge4, badge5, 
          is_featured, live_url, owner_user_id, created_at, is_public,
          client_id, 
          clients ( user_id, name, email ) 
        \'\'\')
      ...
      const { data: clientProjectsData, error: clientError } = await supabase
        ...
        .select(\'''
          ...
          is_featured, live_url, owner_user_id, created_at, is_public,
          client_id, 
          clients!inner ( user_id, name, email )
        \'\'\')
