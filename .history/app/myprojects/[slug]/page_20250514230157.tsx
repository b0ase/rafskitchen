  ...
  const fetchTodoComments = useCallback(async (todoId: string) => {
    ...
    if (commentsError) {
      ...
      const formattedComments = commentsData?.map(c => ({
        ...
        commenter_display_name: c.user?.profiles?.display_name || c.user?.profiles?.username || 'User' 
      })) || [];
  ...
  const handleAddTodoComment = async (todoId: string) => {
    ...
    if (insertError || !newComment) {
      ...
      const formattedComment = {
        ...newComment,
        // @ts-ignore
        commenter_display_name: newComment.user?.profiles?.display_name || newComment.user?.profiles?.username || 'User'
      };
      setComments(prev => ({
        ...prev,
