  ...
  const fetchTodoComments = useCallback(async (todoId: string) => {
    ...
    if (commentsError) {
      ...
      const formattedComments = commentsData?.map(c => ({
        ...c,
        // @ts-ignore supabase type generation might not be perfect for nested profiles
        commenter_display_name: c['auth.users']?.profiles?.display_name || c['auth.users']?.profiles?.username || 'User' 
      })) || [];
  ...
  const handleAddTodoComment = async (todoId: string) => {
    ...
    if (insertError || !newComment) {
      ...
      const formattedComment = {
        ...newComment,
        // @ts-ignore
        commenter_display_name: newComment['auth.users']?.profiles?.display_name || newComment['auth.users']?.profiles?.username || 'User'
      };
      setComments(prev => ({
        ...prev,
