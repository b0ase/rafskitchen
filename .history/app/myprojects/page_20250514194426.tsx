...
export default function MyProjectsPage() {
  ...
  const fetchUserAndProjects = async () => {
    ...
    if (authUser) {
      ...
      // Step 1: Get projects where the user is an approved member
      ...
      if (membershipError) {
        ...
        setError(`Could not load project memberships. DB Error: ${membershipError.message}`);
        // We can still proceed to fetch owned projects, or decide to stop.
        // For now, let's log error and continue to owned projects.
      } else if (membershipData) {
        allProjectIds = allProjectIds.concat(membershipData.map(m => m.project_client_id));
      }

      // Step 2: Get projects the user owns directly
      const { data: ownedProjectIdsData, error: ownedProjectsError } = await supabase
        .from('clients')
        .select('id') // Only select id here to get the list of IDs
        .eq('user_id', authUser.id);

      if (ownedProjectsError) {
        console.error('Error fetching owned projects IDs:', ownedProjectsError);
        setError(`Could not load owned projects. DB Error: ${ownedProjectsError.message}`);
      } else if (ownedProjectIdsData) {
        allProjectIds = allProjectIds.concat(ownedProjectIdsData.map(p => p.id));
      }
      ...
      if (uniqueProjectIds.length > 0) {
        const { data: projectData, error: projectError } = await supabase
          ...
          .in('id', uniqueProjectIds) // Fetch details for all unique project IDs
          .order('created_at', { ascending: false });

        if (projectError) {
          console.error('Error fetching combined projects:', projectError);
        ...
        // No projects owned or member of
        setProjects([]);
      }
    } else {
