  ...
  return (
    <Link href={`/myprojects/${project.project_slug}`} passHref legacyBehavior>
      <a 
        ...
        onClick={(e) => {
          // Prevent navigation if a button or select inside the card was clicked.
          // This relies on those interactive elements stopping propagation.
          if ((e.target as HTMLElement).closest('button, select, a[href*=\"edit\"], a[href*=\"manage-members\"]')) {
            // Let the more specific interactive element handle the click.
            // We could also explicitly call e.stopPropagation() in each of those elements' handlers.
            return; 
          }
          // If the click is not on an interactive element, allow navigation.
        }}
      >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3">
          <div className="flex items-center gap-x-3">
            {/* Project Name - No longer a Link, just text */}
            <span className="text-2xl font-semibold text-sky-400">
              {project.name}
            </span>
            {/* Edit Link - Needs stopPropagation */}
          ...
          {/* Invite Members Button - Needs stopPropagation */}
          {currentUser && project.user_id === currentUser.id && (
            <Link href={`/myprojects/${project.project_slug}/manage-members`} passHref legacyBehavior>
              <a 
                className="mt-2 sm:mt-0 ml-0 sm:ml-auto inline-flex items-center bg-blue-600 hover:bg-blue-500 text-white font-semibold py-1.5 px-3 rounded-md text-xs transition-colors shadow hover:shadow-md"
                onClick={(e) => e.stopPropagation()}
              >
        ...
        {/* View Project Dashboard Text - No longer a Link, just text */}
        ...
        <div className="mt-2 mb-4 space-y-3 sm:space-y-0 sm:flex sm:flex-wrap sm:items-center sm:gap-3">
          <div className="flex-shrink-0">
            ...
            <select 
              ...
              onClick={(e) => e.stopPropagation()} // Already has stopPropagation
            >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-auto pt-3 border-t border-slate-700/50">
          <div className="flex items-center gap-x-2">
            <input 
              ...
              disabled={updatingItemId === project.id}
              className="form-checkbox h-4 w-4 text-sky-600 bg-gray-800 border-gray-600 rounded focus:ring-sky-500 transition duration-150 ease-in-out"
              onClick={(e) => e.stopPropagation()} // Stop propagation for checkbox
            />
          ...
          {currentUser && project.user_id === currentUser.id && (
            <button 
              ...
              disabled={updatingItemId === project.id}
              className="mt-3 sm:mt-0 inline-flex items-center bg-red-700 hover:bg-red-600 text-white font-semibold py-1.5 px-3 rounded-md text-xs transition-colors shadow hover:shadow-md disabled:opacity-50"
            >
