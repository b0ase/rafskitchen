...
export default function MyProjectsPage() {
  ...
  const fetchProjectsAndRoles = useCallback(async () => {
    ...
    try {
      ...
      if (ownedError) {
        console.error('OWNED PROJECTS FETCH ERROR:', ownedError); // More specific error
      ...
      if (clientError) {
        console.error('CLIENT PROJECTS FETCH ERROR:', clientError); // More specific error
        setError(prev => prev ? prev + ' & Failed to fetch client projects.' : 'Failed to fetch client projects.');
      }
      */
      const clientProjectsData: any[] = []; // Empty array to prevent downstream errors
      const clientError = null; // Null error to prevent downstream errors
