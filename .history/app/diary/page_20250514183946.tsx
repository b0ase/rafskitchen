...
export default function DiaryPage() {
  ...
  const handleCreateActionItemsFromEntry = (entryId: string, entryTitle: string, entrySummary: string) => {
    console.log('Create action items for entry ID:', entryId);
    console.log('Title:', entryTitle);
    console.log('Summary:', entrySummary);
    // TODO: Implement modal or UI to create tasks linked to this diary entry
    alert(`Placeholder: Create action items for "${entryTitle}"`);
  };
          ...
          <div className="space-y-8">
            {entries.map(entry => (
              <article key={entry.id} className="bg-gray-900 p-6 md:p-8 border border-gray-800 shadow-xl rounded-lg">
                ...
                <div className="mt-4">
                  <button
                    onClick={() => handleCreateActionItemsFromEntry(entry.id, entry.title, entry.summary)}
