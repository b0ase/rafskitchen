  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main className="px-6 py-12 md:py-16 flex-grow">
        ...
        <section id="hero" className="py-16 md:py-24 text-center bg-gradient-to-br from-gray-900 via-purple-800 to-blue-800 text-white"> {/* Added gradient and base text color */}
          <div className="max-w-6xl mx-auto px-4"> {/* Overall hero container */} 
            <div className="max-w-4xl mx-auto"> {/* Container for text content */}
              <h1 className="text-5xl md:text-7xl font-bold mb-8"> {/* Removed text-gray-900, will inherit text-white */}
                <span className="block text-6xl md:text-8xl font-extrabold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent mb-3 md:mb-4"> {/* Adjusted span gradient for dark bg */}
                  RAFSKITCHEN
                </span>
                Transforming Ideas into Digital Realities.
              </h1>
              <p className="text-xl md:text-2xl text-gray-300 mb-10 md:mb-12"> {/* Changed to text-gray-300 */}
            ...
            {/* Core Platform Features (Integrated) - Title Removed */}
            <div className="mt-0 md:mt-0"> {/* Adjusted top margin */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {studioFeatures.map((feature, index) => (
                  <div
                    key={index}
                    // Removed previous border/bg classes, added new styling for dark theme
                    className={`group p-6 rounded-xl bg-white/10 backdrop-blur-md shadow-xl border border-white/20 transition-all duration-300 hover:bg-white/20 hover:shadow-2xl hover:scale-105`}
                  >
                    <feature.icon className={`text-4xl mb-4 ${studioIconColorClasses[feature.color]}`} /> 
