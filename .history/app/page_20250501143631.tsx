...
const portfolioData = {
  ...
  projects: [
    // New Domain-based Project Stubs (Type: 'domain') - Moved to top
    { ...createProjectStub(7, 'ninjapunkgirls.com', 'Concept for Ninja Punk Girls project.', 'Concept'), type: 'domain' },
    ...
    // Existing GitHub Stubs (Type: 'github') - Now second
    {
      id: 1, 
      title: 'AIOSX',
        ...
        <section id="projects" className="mb-16 scroll-mt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {portfolioData.projects.map((project) => {
              ...
              // Define base styles and conditional styles
              const cardBaseStyle = "p-6 shadow-lg flex flex-col relative group";
              const cardStyle = isDomainProject
                ? `${cardBaseStyle} bg-white text-gray-900` // White card, dark text
                : `${cardBaseStyle} bg-gray-900 text-gray-300`; // Dark card, light text
              ...
              const statusBadgeStyle = isDomainProject ? "bg-gray-200 text-gray-800" : "bg-gray-700 text-gray-300";
              ...
              return (
                <div key={project.id} className={cardStyle}>
                  {/* Status Badge */}
                  <span className={`absolute top-2 right-2 text-xs font-medium px-2.5 py-0.5 z-10 ${statusBadgeStyle}`}>{project.status}</span>
                  ...
                  <div className={`mt-auto pt-4 border-t ${separatorStyle} flex justify-between items-center gap-4`}>
                    ...
                    {project.tokenMarketUrl && project.tokenMarketUrl !== '#' ? (
                      <a
                        ...
                        className={`flex items-center gap-1 text-sm ${tokenLinkStyle}`}
                      >
                        <TokenIcon /> <span className={tokenNameStyle}>{project.tokenName}</span> <span className={tokenPlatformStyle}>({project.tokenPlatform})</span>
                      </a>
                    ) : (
