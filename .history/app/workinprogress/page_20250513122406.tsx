...
export default function WorkInProgressPage() {
  ...
  const handleAddBoaseTask = async (e: FormEvent) => {
    e.preventDefault();
    if (!newBoaseTaskText.trim()) return;
    setErrorAddingTask(null);

    try {
      const { data, error } = await supabase
        .from('b0ase_tasks')
        .insert([{ text: newBoaseTaskText.trim(), status: 'TO_DO', project_scope: 'b0ase.com' }])
        .select();
      ...
      if (error) {
        ...
        if (data) {
          ...
          fetchBoaseTasks(); // Fallback to re-fetch if insert didn't return data as expected
        }
      }
    } catch (e: any) {
