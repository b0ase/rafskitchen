  ...
  const fetchAllUserTasks = async (userId: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        setAllUserTasks(demoTasks);
      } else {
        const fetchedTasks = data || [];
        // If no real tasks, show demo tasks
        setAllUserTasks(fetchedTasks.length > 0 ? fetchedTasks : demoTasks);
      }
    } catch (e: any) {
  ...
  const fetchProjectScopes = async (userId: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Fall back to demo data
        setProjectScopes(demoProjectScopes);
      } else {
        const fetchedScopes = data || [];
        // If no real scopes, show demo scopes
        setProjectScopes(fetchedScopes.length > 0 ? fetchedScopes : demoProjectScopes);
      }
    } catch (e: any) {
  ...
  const fetchMyProjects = async (userId: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        // For demo, create demo projects from our demo scopes
        setMyProjects([
          { id: 'demo-project-1', name: 'RafsKitchen AI Assistant' },
          { id: 'demo-project-2', name: 'Restaurant POS Integration' },
          { id: 'demo-project-3', name: 'Supply Chain Blockchain' }
        ]);
      } else {
        const fetchedProjects = data as MyProject[] || [];
        // If no real projects, show demo projects
