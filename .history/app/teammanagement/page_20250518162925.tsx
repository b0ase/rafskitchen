...
export default function TeamManagementPage() {
  ...
  const [processingHostileTakeover, setProcessingHostileTakeover] = useState<boolean>(false);
  ...
  const handleHostileTakeover = async () => {
    if (!isSuperAdmin || !currentUser?.id) {
      setError('Super admin privileges required for hostile takeover.');
      return;
    }
    const confirmed = window.confirm(
      "Are you ABSOLUTELY sure you want to perform a 'Hostile Takeover'? " +
      "This will attempt to set YOU (" + currentUser.email + ") as the creator AND an owner of ALL teams in the system. " +
      "This is a major operation and may have unintended consequences if RLS policies are not set up to allow it."
    );
    if (!confirmed) return;

    setProcessingHostileTakeover(true);
    setError(null);
    let successCount = 0;
    let failureCount = 0;
    const superAdminUserId = currentUser.id;

    try {
      const { data: allTeamsForTakeover, error: fetchAllTeamsError } = await supabase
        .from('teams')
        .select('id, name, created_by'); // Only need id, name, created_by

      if (fetchAllTeamsError) throw fetchAllTeamsError;

      if (!allTeamsForTakeover || allTeamsForTakeover.length === 0) {
        alert('No teams found in the system to take over.');
        setProcessingHostileTakeover(false);
        return;
      }

      for (const team of allTeamsForTakeover) {
        try {
          // 1. Update created_by in the teams table to super admin
          if (team.created_by !== superAdminUserId) {
            const { error: updateTeamCreatorError } = await supabase
              .from('teams')
              .update({ created_by: superAdminUserId })
              .eq('id', team.id);
            if (updateTeamCreatorError) {
              console.error(`Failed to update creator for team ${team.name} (ID: ${team.id}):`, updateTeamCreatorError);
              throw new Error(`Failed to update creator for ${team.name}: ${updateTeamCreatorError.message}`);
            }
          }

          // 2. Upsert super admin's membership with 'owner' role
          const { error: upsertMembershipError } = await supabase
            .from('user_team_memberships')
            .upsert(
              { team_id: team.id, user_id: superAdminUserId, role: 'owner' },
              { onConflict: 'team_id,user_id' } 
            );
          if (upsertMembershipError) {
            console.error(`Failed to upsert ownership for super admin on team ${team.name} (ID: ${team.id}):`, upsertMembershipError);
            throw new Error(`Failed to upsert ownership for ${team.name}: ${upsertMembershipError.message}`);
          }
          successCount++;
        } catch (singleTeamError: any) {
          console.error(
            `Failed to process hostile takeover for team ${team.name} (ID: ${team.id}):`,
            {
              message: singleTeamError.message,
              code: singleTeamError.code, // May not exist on custom error
              details: singleTeamError.details, // May not exist on custom error
              fullError: singleTeamError
            }
          );
          failureCount++;
      ...
      if (failureCount > 0) {
        setError(`Hostile takeover processed. Successfully took over ${successCount} teams. Failed on ${failureCount} teams. Check console for details. RLS policies might be preventing some operations.`);
      } else {
        alert(`Hostile takeover complete! Successfully asserted control over ${successCount} team(s).`);
      }
      fetchAllTeams(); // Refresh the list
    } catch (e: any) {
      console.error('Error during hostile takeover:', e);
      setError(`Failed to perform hostile takeover: ${e.message}. This could be due to RLS policies or other database constraints.`);
    } finally {
      setProcessingHostileTakeover(false);
    }
  };
