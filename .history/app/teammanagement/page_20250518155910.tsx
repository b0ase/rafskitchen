      ...
      if (teamDeleteError) {
        ...
        if (teamDeleteError.message.includes('violates foreign key constraint')) {
          setError(
            ...
            `Please ensure cascading deletes are set up correctly in the database for 'user_team_memberships' and 'team_messages' tables, ` +
            `or remove them manually first. Original error: ${teamDeleteError.message}`
          );
        } else {
          // For other errors, re-throw to be caught by the generic catch block
          throw teamDeleteError; 
        }
        // No need to return here, error state is set, finally block will run
      } else {
    ...
    try {
      ...
      for (const team of orphanedTeams) {
        try {
          ...
          console.error(`Failed to process orphaned team ${team.name} (ID: ${team.id}):`, singleTeamError);
          failureCount++;
        }
      }
