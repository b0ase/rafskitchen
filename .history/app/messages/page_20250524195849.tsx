    ...
    try {
      ...
      const processedTeamsData = await Promise.all(
        teamsData.map(async (team) => {
          let unreadCount = 0;
          const defaultColorScheme: ColorScheme = { bgColor: 'bg-white', textColor: 'text-gray-900', borderColor: 'border-gray-200' };
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row md:space-x-8">
        ...
        <div className="w-full md:w-1/2 mb-8 md:mb-0">
          <h2 className="text-2xl font-semibold text-black mb-4">Direct Messages</h2>
          {directThreads.length > 0 ? (
            <div className="space-y-4">
              {directThreads.map(user => (
                <Link
                  ...
                  className="block p-4 bg-white border border-gray-200 rounded-lg hover:border-gray-300 transition-colors flex items-center space-x-4 shadow-lg"
                >
                  {user.avatar_url ? (
                  ...
                  <div>
                    <span className="text-lg text-black">{user.display_name || 'Unknown User'}</span>
                    {/* Placeholder for unread indicator */}
                    {user.unread_count > 0 && (
                      <span className="ml-2 px-2 py-0.5 bg-cyan-600 text-white text-xs font-semibold rounded-full">
                        {user.unread_count}
                      </span>
                    )}
                  </div>
            ...
            !loading && ( // Only show "No direct messages" if not loading
              <div className="text-center py-10 bg-white border border-gray-200 rounded-lg shadow-lg">
                ...
                <p className="text-gray-600">No direct messages.</p>
              </div>
            )
          )}
        </div>
        ...
        <div className="w-full md:w-1/2">
          <h2 className="text-2xl font-semibold text-black mb-4">Team Messages</h2>
          {userTeams.length > 0 ? (
            <div className="space-y-4">
              {userTeams.map((team) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center p-4 rounded-lg border transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-xl ${cardBgColor} ${cardTextColor} ${cardBorderColor} shadow-lg`}
                  >
                    ...
                    {/* Placeholder for unread indicator */}
                    {team.unread_count > 0 && (
                      <span className="ml-auto px-2.5 py-1 bg-cyan-600 text-white text-xs font-bold rounded-full self-center">
                        {team.unread_count}
                      </span>
                    )}
                  </Link>
             ...
             !loading && ( // Only show "You are not part of any teams" if not loading
              <div className="text-center py-10 bg-white border border-gray-200 rounded-lg shadow-lg">
                ...
                <p className="text-gray-600">You are not part of any teams.</p>
      ...
      <div className="mt-12 bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        ...
        <p className="text-yellow-700">
          You're currently viewing a demo version of the RafsKitchen messages dashboard. 
          ...
          and collaborate with other platform users.
        </p>
        <div className="mt-4">
          <Link
            href="/"
