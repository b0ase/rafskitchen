              ...
              <div className="space-y-10">
                      ...
                      {groupedSkills[category].map(skill => {
                        ...
                        return (
                          <button
                            ...
                            className={`${getSkillBadgeStyle(skill.category, isSelected)} ${isCurrentlySaving ? 'opacity-60 cursor-wait' : ''}`}
                            title={skill.description || skill.name}
                          >
                            {isCurrentlySaving ? <FaSpinner className="animate-spin mr-2 text-lg" /> : (isSelected ? <FaCheckCircle className="mr-2 text-lg text-green-300" /> : <FaPlusCircle className="mr-2 text-lg text-sky-300" />)}
                            {skill.name}
                          </button>
                        );
                      })}
                ...
                {allSkills.length === 0 && !loadingSkills && (
                   <div className="text-center py-10 px-6 border-2 border-dashed border-gray-700 rounded-lg bg-gray-800/50">
                      ...
                      <p className="text-gray-400 text-xl mb-2">No skills found.</p>
                      <p className="text-gray-500">Consider seeding your database or check back later.</p>
                   </div>
                )}
              </div>
            )}
