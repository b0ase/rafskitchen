...
import { FaExternalLinkAlt, FaGithub, FaInfoCircle, FaArrowRight, FaLinkedin } from 'react-icons/fa';
    ...
    <div className="text-gray-300 flex flex-col">
      ...
      <main className="container mx-auto px-4 py-12 md:py-16 flex-grow">
        ...
        <section id="about" className="mb-16 md:mb-24 scroll-mt-20">
          {/* Dark card style */}
          <div className="bg-black p-6 md:p-8 border border-gray-800 shadow-xl">
            {/* Dark text styles */}
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">{portfolioData.about.name}</h1>
            <p className="text-lg md:text-xl text-gray-300 mb-4">{portfolioData.about.tagline}</p>
            <p className="text-base text-gray-400 mb-6 whitespace-pre-line">{portfolioData.about.bio}</p>
            ...
            <div className="bg-gray-900 p-4 border border-gray-700 text-sm text-gray-400">
              ...
              {/* Button style consistent with dark theme */}
              <a 
                href={portfolioData.about.token.marketLink}
                target="_blank" 
                rel="noopener noreferrer"
                className="inline-flex items-center px-4 py-2 bg-blue-700 text-white text-xs font-medium hover:bg-blue-600 transition-colors shadow-md"
              >
        ...
        <section id="services" className="mb-16 md:mb-24 scroll-mt-20">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 border-b border-gray-800 pb-2">Services</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {portfolioData.services.map((service) => (
              // Dark card style
              <div key={service.id} className="bg-black p-6 border border-gray-800 shadow-xl flex flex-col">
                <h3 className="text-xl font-semibold text-white mb-2">{service.title}</h3>
                <p className="text-gray-400 text-sm mb-4 flex-grow">{service.description}</p>
                {service.priceInfo && (
                  <p className="text-xs text-gray-500 italic mt-auto pt-2 border-t border-gray-700">{service.priceInfo}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <section id="skills" className="mb-16 md:mb-24 scroll-mt-20">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 border-b border-gray-800 pb-2">Technical Skills</h2>
          <div className="flex flex-wrap gap-2">
            {portfolioData.skills.technical.map((skill) => (
              // Dark badge style
              <span key={skill} className="bg-gray-800 text-gray-300 text-xs font-medium px-2.5 py-0.5 border border-gray-700 shadow-sm">
                {skill}
              </span>
            ))}
          </div>
        ...
        {/* Projects (Domains) Section - Dark Theme */}
        <section id="projects" className="mb-16 md:mb-24 scroll-mt-20">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 border-b border-gray-800 pb-2">Client Projects</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.filter(p => p.type === 'domain').map((project) => (
              // Dark card style
              <div key={project.id} className="bg-black border border-gray-800 shadow-xl flex flex-col overflow-hidden group">
                {project.imageUrl && (
                ...
                <div className="p-5 flex flex-col flex-grow">
                  ...
                  {/* Status/Token Badge - Dark Theme */}
                  {project.status && (
                    <span className={`text-xs font-medium mr-2 px-2 py-0.5 mb-2 inline-block w-max border 
                      ${project.status === 'Live' ? 'bg-green-900 border-green-700 text-green-300' : 
                       project.status === 'In Development' ? 'bg-yellow-900 border-yellow-700 text-yellow-300' : 
                  ...
                  {project.tokenName && (
                    <span className="bg-purple-900 border-purple-700 text-purple-300 border text-xs font-medium mr-2 px-2 py-0.5 mb-2 inline-block w-max">
                  ...
                  {typeof project.tokenProgressPercent === 'number' && (
                    <div className="w-full bg-gray-700 h-1 mb-3">
                      <div className="bg-blue-600 h-1" style={{ width: `${project.tokenProgressPercent}%` }}></div>
                    </div>
                  )}
                  ...
                  <div className="mt-auto pt-3 border-t border-gray-700 flex items-center justify-between">
                      <div className="flex space-x-3">
                          {project.liveUrl && (
                              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 transition-colors" title="Visit Live Site">
                                  <FaExternalLinkAlt size={16} />
                              </a>
                          )}
                          {project.repoUrl && (
                              <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" title="View Repository">
                                  <FaGithub size={16} />
                              </a>
                          )}
                      </div>
                      {/* Details Link - Dark Theme */}
                      <Link href={`/projects/${project.slug}`} className="text-xs text-blue-400 hover:text-blue-300 font-medium inline-flex items-center">
        ...
        {/* Development (GitHub Repos) Section - Dark Theme */}
        <section id="development" className="mb-16 md:mb-24 scroll-mt-20">
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 border-b border-gray-800 pb-2">Open Source / Development</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {development.filter(p => p.type === 'github').map((project) => (
                    // Dark card style
                    <div key={project.id} className="bg-black p-6 border border-gray-800 shadow-xl flex flex-col">
                        <h3 className="text-lg font-semibold text-white mb-1">{project.title}</h3>
                        <p className="text-sm text-gray-400 mb-3 flex-grow">{project.description}</p>
                        ...
                        {project.status && (
                           <span className={`text-xs font-medium mr-2 px-2 py-0.5 mb-2 inline-block w-max border
                           ${project.status === 'Live' ? 'bg-green-900 border-green-700 text-green-300' : 
                           project.status === 'Archived' ? 'bg-gray-800 border-gray-700 text-gray-400' : 
                        ...
                        {/* Links - Dark Theme */}
                        <div className="mt-auto pt-3 border-t border-gray-700 flex space-x-3">
                            {project.liveUrl && (
                                <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 transition-colors" title="Visit Live Site">
                                    <FaExternalLinkAlt size={16}/>
                                </a>
        ...
        <section id="contact" className="mb-16 md:mb-24 scroll-mt-20">
             {/* Dark card style */}
            <div className="bg-black p-6 md:p-8 border border-gray-800 shadow-xl text-center">
                ...
                <a 
                    ...
                    className="inline-flex items-center px-6 py-3 bg-blue-700 text-white text-base font-medium hover:bg-blue-600 transition-colors shadow-md"
                >
                   Contact B0ASE
                </a>
