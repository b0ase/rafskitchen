...
export default function TeamPage() {
  ...
  const fetchDisplayProjects = useCallback(async (userId: string) => {
    ...
    try {
      // Step 1: Fetch all projects the user *might* have access to based on 'projects' table RLS
      // RLS on 'projects' table should filter based on ownership, membership, or public status.
      const { data: accessibleProjectsData, error: projectsError } = await supabase
        ...
        // Note: No .in() filter here initially; we let RLS on 'projects' do its job.

      if (projectsError) throw projectsError;
      if (!accessibleProjectsData || accessibleProjectsData.length === 0) {
        setDisplayProjects([]);
        setIsLoadingDisplayProjects(false);
        return;
      }

      // Step 2: For each accessible project, determine the current user's role.
      const projectsToDisplay = await Promise.all(
        accessibleProjectsData.map(async (proj) => {
          let userRoleForThisProject: string | null = null;

          if (proj.owner_user_id === userId) {
            userRoleForThisProject = ProjectRole.Owner; // Enum value
          } else {
            // Check project_members for a specific role for this user in this project
            ...
            if (memberRoleError && memberRoleError.code !== 'PGRST116') { // PGRST116: no row found
              console.warn(`Error fetching member role for project ${proj.id} for user ${userId}:`, memberRoleError.message);
            } else if (memberRoleData && memberRoleData.role) {
              userRoleForThisProject = memberRoleData.role; // This would be 'Client', 'Freelancer', etc. from DB
            }
          }
          
          // If after checking ownership and project_members, the user still has no specific role,
          // but the project was returned by the initial query (meaning RLS on 'projects' allowed it),
          // it implies they have access (e.g., it's public or RLS on projects has another rule).
          // We will only display projects where the user is owner OR has a role in project_members.
          // Or if it's public and we want to show a generic role like 'Viewer' (but that's not the current goal for this section)

          if (!userRoleForThisProject && !proj.is_public) { // If not owner, not in project_members, and not public, skip (unless RLS on projects has other view criteria)
             // However, the initial query on 'projects' already respects RLS. If a project is here, the user CAN see it.
             // The question is what role to display. If no specific role, and RLS on projects allows view, what then?
             // For this section, we primarily care about projects they have a defined relationship with.
             // Let's stick to showing projects where they are owner or have a role in project_members for now.
             // This logic might need refinement based on exact RLS on 'projects' and desired visibility.
          }
          
          // Only include the project if the user is owner or explicitly a member with a role
          // Or if we decide to show public projects they are not directly a member of (e.g. as a 'Viewer')
          if (proj.owner_user_id === userId || userRoleForThisProject) {
            return {
              ...
              currentUserRole: userRoleForThisProject, // This will be Owner, or role from project_members
              project_category: proj.project_category,
            } as DisplayProject;
          } else {
            return null; // Don't include this project in the display list if no direct role/ownership for this specific view
          }
        })
      );
      // Filter out nulls for projects the user isn't directly associated with for this view's purpose
      setDisplayProjects(projectsToDisplay.filter(p => p !== null) as DisplayProject[]); 
