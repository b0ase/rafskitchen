                          ...
                          {teamMembers.map(member => (
                            <li key={member.user_id} className="flex justify-between items-center p-2.5 bg-gray-800 rounded-md shadow">
                              <div>
                                ...
                                <span className={`ml-3 text-xs uppercase px-2 py-1 rounded-full font-semibold
                                  ${member.role.toLowerCase() === ProjectRole.ProjectManager || member.role.toLowerCase() === 'project_manager' ? 'bg-sky-700/70 text-sky-200' :
                                    member.role.toLowerCase() === ProjectRole.Freelancer || member.role.toLowerCase() === 'collaborator' ? 'bg-indigo-700/70 text-indigo-200' :
                                    member.role.toLowerCase() === ProjectRole.CLIENT || member.role.toLowerCase() === 'clientcontact' ? 'bg-green-700/70 text-green-200' :
                                    member.role.toLowerCase() === ProjectRole.Viewer ? 'bg-gray-600/70 text-gray-100' :
                                    'bg-gray-700/70 text-gray-200' // Default/fallback
                                  }`}>
