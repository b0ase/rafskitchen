      ...
      const displayProjectsWithDetails = await Promise.all(projectsData?.map(async (project) => {
        ...
        if (project.owner_user_id) {
          ...
          if (ownerProfileError && ownerProfileError.code !== 'PGRST116') {
            console.error('Error fetching owner profile for project', project.name, ':', ownerProfileError);
          }
          if (ownerProfile) {
            teamLeader = { display_name: ownerProfile.display_name || ownerProfile.username || 'Unnamed Owner' };
          }
        }
        ...
        const { data: membersData, error: membersError } = await supabase
          .from('project_members') // MODIFIED: from 'project_users'
          .select('user_id, role, profiles(display_name, username)') // Fetch role and profile directly
        ...
        return {
          ...
          name: project.name,
          slug: project.slug,
          icon_name: null, // TODO: Determine source for icon_name or remove
          color_scheme: null, // TODO: Determine source for color_scheme or remove
          team_leader: teamLeader,
          token: null, // TODO: Re-evaluate token fetching if still needed
          client_name: project.name, // Or map from a specific client relationship if established
          project_type: project.project_category,
          team_members: fetchedTeamMembers.map(m => ({ display_name: m.display_name})), // Simplified for now
          project_data: { id: project.id, name: project.name }, // Simplified ManagedProject
        } as DisplayProject;
      }) || []);
