            ...
            {!isLoadingUserTeams && !errorFetchingUserTeams && userTeams.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {userTeams.map((team) => {
                  ...
                  const cardTextColor = 'text-gray-300';

                  return (
                    <Link key={team.id} href={`/teams/${team.slug || team.id}`} passHref legacyBehavior>
                      <a className={`block rounded-lg shadow-md p-4 flex flex-col justify-between border transition-all duration-200 ease-in-out hover:border-gray-700 ${cardBgColor} ${cardBorderColor} ${cardTextColor} cursor-pointer`}>
                        <div className="flex items-center mb-4">
                          <IconComponent className={`text-xl mr-3 ${cardTextColor} opacity-90`} />
                          <h3 className={`text-base font-normal ${cardTextColor} truncate`}>{team.name}</h3>
                        </div>
                        <div className="mt-auto pt-4 text-right">
                          <span className={`text-xs ${cardTextColor} opacity-70 hover:opacity-100`}>Open Chat <FaAngleRight className="inline ml-1" /></span>
                        </div>
                      </a>
                    </Link>
                  );
