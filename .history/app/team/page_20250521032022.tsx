...
export default function TeamPage() {
  ...
  // --- NEW Function to Fetch User's Teams with related Project, Leader, and Token ---
  const fetchUserTeams = useCallback(async (userId: string) => {
    if (!userId) return;
    setIsLoadingUserTeams(true);
    setErrorFetchingUserTeams(null);
    try {
      // Step 1: Get team_ids and associated project_ids (indirectly via user memberships)
      // This requires joining user_team_memberships and project_users, which might not directly
      // link a specific team to a specific project in a 1:1 manner via user alone.
      // A more robust schema would have a direct team_id on the clients (projects) table.
      // For now, we'll fetch teams and then attempt to find a related project the user manages or is part of
      // for each team. This might not be accurate if a user is in one team but manages unrelated projects.
      // A better approach might require a database schema change.

      // Let's fetch user's team memberships
      ...
      // Step 2: Fetch details for these teams
      ...
      // Step 3: For each team, try to find a related project, team leader, and token.
      // This is a simplified approach due to schema structure. A direct link is preferred.
      const teamsWithDetails = await Promise.all(teamsData?.map(async (team) => {
        let relatedProject = null;
        let teamLeader = null;
        let projectToken = null;

        // Attempt to find a project linked to this team *via the current user* or team members
        // This is heuristic and might need adjustment based on actual data relationships.
        // A more reliable way might be to query `project_users` for users in this team,
        // and then find projects those users are project_managers for.
        // Or, if a team is tied to ONE project, query `project_users` for any member of this team
        // who is a project manager of any project, and pick one (or the one the team is for).

        // Let's try fetching project the current user is a manager of and see if any team member is part of it
        // This is complex client-side. A database function or view would be better.

        // Simplified heuristic: Find projects the current user is a manager of
        const { data: managedProjectsData, error: managedProjectsError } = await supabase
          .from('project_users')
        ...
        if (managedProjectsError) {
          console.error('Error fetching managed projects for team linking:', managedProjectsError);
        }

        if (managedProjectsData && managedProjectsData.length > 0) {
          // Assuming for simplicity, the *first* managed project is related to the team
          // This is a strong assumption due to schema limitations.
          const firstManagedProjectId = managedProjectsData[0].project_id;

          // Fetch project details from the 'clients' table
          const { data: projectData, error: projectError } = await supabase
            .from('clients')
            .select('id, name, slug, project_category')
            .eq('id', firstManagedProjectId)
            .single();

          if (projectError && projectError.code !== 'PGRST116') { // PGRST116: no rows found
             console.error('Error fetching project details for team:', projectError);
          }
          ...
          if (projectData) {
            ...
            if (projectManagers && projectManagers.length > 0) {
               ...
               const firstPmId = projectManagers[0].user_id;
               ...
               if (pmProfileError && pmProfileError.code !== 'PGRST116') {
                 console.error('Error fetching project manager profile:', pmProfileError);
               }

               if (pmProfile) {
                 teamLeader = { display_name: pmProfile.display_name || 'Unnamed User' };
               }
            }
            ...
            const { data: tokenData, error: tokenError } = await supabase
              ...
              // Assuming a token is linked to a project by project_id, but tokens table has user_id and token_category...
              // Let's assume for now a token with category 'project' and linked to the project manager's user_id might represent the project token.
              // A better approach would be a direct project_id on the tokens table.
              // For this heuristic, let's try linking via the project manager's user_id and category 'project'
               .eq('user_id', teamLeader?.display_name ? projectManagers?.[0]?.user_id : userId) // Use project manager or current user id as heuristic
               .eq('token_category', 'project')
               .single();

            if (tokenError && tokenError.code !== 'PGRST116') {
               console.error('Error fetching token for project:', tokenError);
            }
        ...
        // Process teamData to add default color_scheme and icon_name if null
        const processedTeam = {
          ...team,
                    ...
                    <Link key={team.id} href={`/teams/${team.slug || team.id}`} passHref legacyBehavior>
                      <a className={`block rounded-md shadow-none p-3 flex flex-col justify-between border transition-all duration-200 ease-in-out hover:border-gray-600 ${cardBgColor} ${cardBorderColor} ${cardTextColor} cursor-pointer`}>
                        ...
                        <div className="text-gray-400 text-xs space-y-0.5 mb-2">
                          {team.team_leader && <p>Team Leader: {team.team_leader.display_name}</p>}
                          {team.project && <p>Project: {team.project.name}</p>}
                          {team.project && <p>Client: {team.project.name}</p>}
                          {team.token && <p>$ Token: {team.token.ticker_symbol}</p>}
                          {team.project?.project_category && <p>Project Type: {team.project.project_category}</p>}
                          {!team.team_leader && !team.project && !team.token && !team.project?.project_category && (
