          ...
          <div className="space-y-8">
            {learningPath.map((week) => (
              <div key={week.id} className="relative pl-16 md:pl-20">
                {/* Timeline dot */}
                <div className={`absolute left-4 md:left-6 top-2 w-5 h-5 rounded-full border-4 border-gray-900 bg-${week.color}-500`}></div>
                
                {/* Content */}
                <div className={`p-4 border border-gray-700 rounded-lg bg-gray-850 hover:border-${week.color}-500/50 transition-colors`}>
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-3">
                    <h4 className={`text-lg font-semibold text-${week.color}-400`}>{week.platform}: {week.focus}</h4>
