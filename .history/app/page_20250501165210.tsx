              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    {/* Separator - Lighter color for visibility */}
                    <hr className={`my-4 border-gray-500`} />

                    {/* External Links - Lighter color for visibility */}
                    <div className="flex justify-start space-x-3 mb-4">
                      {project.githubUrl && project.githubUrl !== '#' && (
                        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className={`text-gray-300 hover:text-white transition-colors`} aria-label={`${project.title} GitHub Repository`}>
                          <GitHubIcon />
                        </a>
                      )}
                      {project.xUrl && project.xUrl !== '#' && (
                        <a href={project.xUrl} target="_blank" rel="noopener noreferrer" className={`text-gray-300 hover:text-white transition-colors`} aria-label={`${project.title} X.com Profile`}>
                          <XIcon />
                        </a>
                      )}
                      {project.notionUrl && project.notionUrl !== '#' && (
                        <a href={project.notionUrl} target="_blank" rel="noopener noreferrer" className={`text-gray-300 hover:text-white transition-colors`} aria-label={`${project.title} Notion Page`}>
                          <NotionIcon />
                        </a>
                      )}
                    </div>

                    {/* Token Info - Lighter color for visibility */}
                    {project.tokenName && (
                      <div className="flex justify-between items-center text-sm mt-auto"> {/* Push to bottom */} 
                        <span className={`font-semibold text-gray-100`}>{project.tokenName}</span>
                        {/* Display progress bar instead of just percentage */}
                           ...
                           <div className="flex items-center ml-2">
                             <div className="w-16 h-2 bg-gray-600 rounded-full overflow-hidden mr-2">
                               <div 
                                 ...
                                 style={{ width: `${100 - project.tokenProgressPercent}%` }} // Bar shows SOLD amount
                               ></div>
                             </div>
                             <span className={`text-xs text-gray-300`}>{project.tokenProgressPercent}% Available</span>
                           </div>
                        )}
                      </div>
                    )}
