...
export async function POST(request: NextRequest) {
  ...
  const {
    ...
    socialLinks,
    inspiration_links,
    // how_heard, // Not directly stored in primary tables yet, can be added to projects if needed
    addProjectTeam,
    addProjectToken,
    addProjectAgent,
    addProjectWebsite,
  ...
  try {
    ...
    if (existingClient) {
      ...
      // Optionally, update existing client record with new info if needed
      // For now, we assume if client exists, their core details are set.
    } else {
      const { data: newClient, error: clientInsertError } = await supabase
        .from('clients')
        .insert({
          user_id: user.id,
          name: user.email || 'New Client', // Or use a name from user profile if available
          email: user.email,
          project_slug: projectSlug, // This seems specific to one project, consider general client slug or removing
          website: website_url || desired_domain_name, // Prioritize existing website_url
          logo_url: logo_url,
          project_brief: project_brief, 
          project_category: project_type,
          // status: 'pending' // Default status for a new client
        })
        .select('id')
        .single();
      ...
      if (clientInsertError || !newClient) {
        ...
        return NextResponse.json({ error: 'Failed to create client record.', details: clientInsertError?.message }, { status: 500 });
      }
    ...
    const { data: newProject, error: projectError } = await supabase
      ...
      .insert({
        ...
        url: website_url || desired_domain_name, // Storing the primary URL for the project
        budget: isNaN(Number(budget)) ? null : budget,
        status: 'pending_setup', // Initial status
        // what_to_build: what_to_build, // Add if you have a corresponding column in projects
        // project_type: project_type, // Add if you have a corresponding column in projects
        // logo_url: logo_url, // Add if you have a corresponding column in projects
        social_links: socialLinks, // Storing the socialLinks object directly as JSONB
        inspiration_links: inspiration_links, // Storing inspiration links as text
      })
      .select('id, slug')
      .single();
    ...
    if (addProjectTeam) {
      ...
      if (teamError) {
        ...
        // Decide if this is a critical failure or if you can continue
      }
    }
    ...
    if (addProjectAgent) {
      ...
      if (agentError) {
        console.error('Error creating project agent:', agentError.message);
      }
    }

    if (addProjectWebsite) {
      // Use desired_domain_name for the git_repository_url or a more specific website configuration
      // The actual live URL is already stored in projects.url
      const websiteGitRepo = (desired_domain_name && desired_domain_name.startsWith('https://github.com')) 
                            ? desired_domain_name 
                            : (socialLinks?.github || null);

      const { error: websiteError } = await supabase.from('project_websites').insert({
        project_id: projectId,
        url: website_url || desired_domain_name || `https://${projectSlug}.b0ase.com`, // Live/Primary URL for the website entity itself
        git_repository_url: websiteGitRepo 
      });
      if (websiteError) {
        console.error('Error creating project website:', websiteError.message);
      }
    }
