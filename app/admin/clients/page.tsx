    ...
    <div className="max-w-5xl mx-auto my-12 p-6 bg-gray-900 text-white rounded-xl shadow-lg">
      ...
      {selected && (
        <div className="bg-gray-800 p-6 rounded-xl mb-8">
          ...
          <textarea className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded mt-2 mb-2" placeholder="Review notes (private)" value={reviewNotes} onChange={e => setReviewNotes(e.target.value)} />
          {selected.status === "pending" && (
            <div className="flex gap-4 mt-4">
              <button className="bg-green-700 hover:bg-green-600 text-white font-bold py-2 px-4 rounded" onClick={() => handleApprove(selected.id)} disabled={loading}>Approve</button>
              <button className="bg-red-700 hover:bg-red-600 text-white font-bold py-2 px-4 rounded" onClick={() => handleReject(selected.id)} disabled={loading}>Reject</button>
              <textarea className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded ml-4" placeholder="Rejection reason (will be emailed)" value={rejectionReason} onChange={e => setRejectionReason(e.target.value)} />
            </div>
          )}
          <button className="mt-4 text-blue-400 underline" onClick={() => setSelected(null)}>Close</button>
        </div>
      )}
    </div>
  );
