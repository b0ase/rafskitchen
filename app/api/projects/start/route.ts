...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (addProjectWebsite) {
      const gitRepoUrl = socialLinks?.github || null; // Get GitHub URL from socialLinks or null
      const productionHostUrl = website_url || desired_domain_name || null;

      const { error: websiteError } = await supabase
        .from('project_websites')
        .insert({
          project_id: projectId,
          url: productionHostUrl, 
          git_repository_url: gitRepoUrl,
          hosting_details: productionHostUrl ? { production_url: productionHostUrl } : null 
        })
        .select('id')
        .single();

      if (websiteError) {
        console.error('Error creating project website entry:', websiteError.message);
        // Decide on error handling: e.g., add to a list of non-critical errors
        // but allow project creation to succeed if website entry fails.
      }
    }
